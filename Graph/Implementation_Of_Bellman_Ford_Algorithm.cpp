...
vi Bellman_Ford(int v, int src, vvi edges)
{
    ...
    fo(i, 0, v - 1)
    {
        fr(edge, edges)
        {
            ...
            if (dist[u] != INT_MAX and dist[u] + wt < dist[v]) // Possible oveflow handled
                dist[v] = dist[u] + wt;
        }
    }
    // Negative wt cycle handling
    fr(edge, edges)
    {
        int u = edge[0], v = edge[1], wt = edge[2];
...
void solve()
{
    ...
    // Edge list representation required
    vvi edges; //(a,b,3),(c,d,5).....
    ...
    vi distances = Bellman_Ford(n, 1, edges);
    fo(i, 1, n+1)cout<< "Node " << i << " is at distance " << distances[i] << nl;
}
int main()
{
    fast;
