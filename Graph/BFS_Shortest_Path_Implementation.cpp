...
public:
    ...
    {
        // BFS Can determine shortest path of an unweighted graph
        // Single source shortest path algorithm for undirected graph
        queue<int> q;
        vector<bool> vis(V, false);
        vector<int> dist(V, 0);
        vector<int> parent(V, -1); // To trace out the shortest path
        ...
        fo(i, 0, V) cout << "Shortest Distance to node " << i << " is " << dist[i] << nl;

        // Print the path from a source to destination
