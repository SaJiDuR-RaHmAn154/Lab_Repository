...
public:
    ...
    // We will call dfs and check whether a child returns if there exists a "Backedge" or not
    // If a backedge is found then the directed graph has a cycle
    // We will maintain a visited array and a stack(Whether a node is visited in the current branch or not)
    ...
    bool dfs(int node, vector<bool> &vis, vector<bool> &stack)
    {
        ...
        // Do some work at a node,return true if backedge is found here itself
        for (auto nbr : l[node])
        {
            if (stack[nbr])
                return true;
            ...
            {
                bool nbrFoundACycle = dfs(nbr, vis, stack);
                if (nbrFoundACycle)
                    return true;
            }
        }
