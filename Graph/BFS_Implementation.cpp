...
public:
    ...
    void BFS(int source)
    {
        // Level order traversing(Breadth First Manner)
        // Lower level nodes will be printed first
        ...
        while (!q.empty())
        {
            // Do some work for each node

            int f = q.front();
            cout << f << "->";
            q.pop();

            // Push the nbrs of the current node inside q if they are not visited
