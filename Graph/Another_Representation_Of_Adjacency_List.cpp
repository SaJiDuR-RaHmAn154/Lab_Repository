...
{
    // All node
    // Hashmap(string(key),Node*(address of the particular node))
    // Directed graph
    // This implementation is used when node object is complex
...
public:
    ...
    void addEdge(string city_x, string city_y, bool undir = false)
    {
        m[city_x]->neighbours.push_back(city_y);
        if (undir)
            m[city_y]->neighbours.push_back(city_x);
    }
    ...
    void printAdjList()
    {
        for (auto cityPair : m)
        {
            auto city = cityPair.ff;
            Node *node = cityPair.ss;
            cout << city << "->";
