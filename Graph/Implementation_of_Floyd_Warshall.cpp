...
vvi Floyd_Warshall(vvi graph)
{
    vvi dist(graph);
    int V = sz(graph);
    // Phases,in k-th phase we include vertices 1,2,...k as intermediate vertices
    ...
    fo(k, 0, V)
    {
        ...
        fo(i, 0, V)
        {
            fo(j, 0, V)
            {
                // if vertex k is included and can we minimize the distance by taking k as an intermediate vertex
...
void solve()
{
    ...
    vvi graph = {
        {0, INF, -2, INF}, // IF there is no direct edge,then it is initialized as INF
        {4, 0, 3, INF},
        {INF, INF, 0, 2},
        {INF, -1, INF, 0}};
