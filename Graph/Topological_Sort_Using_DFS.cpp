...
public:
    ...
    // Make dfs call and reach leaf node,then while coming back add the visited the node in front of the doubly linked list
    // If any vertex is left unvisited and make dfs call on that

    void dfs(int node, vector<bool> &vis, list<int> &ordering)
    {
        vis[node] = true;
        ...
        {
            if (!vis[nbr])
                dfs(nbr, vis, ordering);
        }
        // Backtracking at this point
    ...
    void dfs_topological_sort()
    {
        ...
        // We will call dfs from every node which is not visited
        fo(i, 0, V)
        {
            // One dfs will run for 0 and one for 1 in this example
            if (!vis[i])
                dfs(i, vis, ordering);
        }
