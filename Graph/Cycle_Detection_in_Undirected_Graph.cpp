...
public:
    ...
    bool dfs(int node, vector<bool> &vis, int par)
    {
        ...
        for (auto nbr : l[node])
        {
            if (!(vis[nbr]))
            {
                bool nbrFoundACycle = dfs(nbr, vis, node);
                if (nbrFoundACycle)
                    return true;
            }
            // This neighbour is visited and its not the parent of this current node in the current dfs call
            else if (nbr != par)
                return true;
        }
        return false;
    ...
    bool contains_cycle()
    {
        ...
        // Assuming that this graph is a single component(No disconnected nodes exit)
