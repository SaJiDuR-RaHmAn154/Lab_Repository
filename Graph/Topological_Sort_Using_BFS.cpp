...
public:
    ...
    // Topological sort is used to find the order of the task completed
    // We will maintain a queue
    void topological_sort()
    {
        // First calculate indegree for each vertex
        ...
        queue<int> q;
        // Initialize the queue with the nodes having 0 indegree
        // 0 indegree refers to no dependencies
        fo(i, 0, V)
        {
            if (!indegree[i])
                q.push(i);
        ...
        while (!q.empty())
        {
            ...
            // Iterate over the nbrs of this node and reduce the indegree of their indegree by a factor 1
            for (auto nbr : l[node])
            {
                indegree[nbr]--;//When the node is popped off from the queue we reduce the indegree of its childs
                if (!indegree[nbr])
                    q.push(nbr);
            }
        }
